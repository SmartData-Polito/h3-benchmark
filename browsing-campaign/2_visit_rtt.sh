...
sudo docker network rm rtt_$RTT
sudo docker network create  --driver bridge \
                            --opt "com.docker.network.bridge.name"="docker_rtt_${RTT}" rtt_$RTT
sudo tc qdisc add dev docker_rtt_${RTT} root netem delay ${RTT}ms
...
#for domain in www.youtube.com ; do
    ...
    # HTTP 1
    sudo docker run --network=rtt_$RTT --rm -v "$(pwd)"/${OUTDIR}:/results sitespeedio/browsertime:10.9.0 \
                                                                                --video \
                                                                                --visualMetrics \
                                                                                -n $N \
                                                                                --useSameDir \
